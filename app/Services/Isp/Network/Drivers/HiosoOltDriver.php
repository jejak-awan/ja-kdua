...
class HiosoOltDriver extends BaseSshDriver implements OltDriver
{
    ...
    protected function initializeSession(): void
    {
        ...
        $this->writeAndRead('terminal page-break disable', '#');
    }
    ...
    public function registerONU(string $sn, array $config): bool
    {
        ...
        $commands = [
            ...
            "ont vlan {$onuId} mode tag {$vlan}",
            'exit',
            'save',
        ];

        Log::info("HIOSO OLT [{$this->olt->name}]: Registering ONU {$sn} on 0/{$port}:{$onuId}");
    ...
    public function getSignal(string $interface, string $onuIndex): ?float
    {
        ...
        $cmd = "show ont optical-info epon {$interface} {$onuIndex}";
    ...
    protected function executeOltCommands(array $commands, string $prompt = '#'): bool
    {
        ...
        foreach ($commands as $command) {
            Log::debug("HIOSO OLT [{$this->olt->name}]: Executing -> {$command}");
