...
class VoucherService
{
     ...
     */
    public function useVoucher(string $code, ?int $userId = null): bool
    {
    ...
    public function getSalesReport(int $year, int $month, ?int $partnerId = null): array
    {
        ...
        $results = $query
            ->selectRaw('used_at::date as date, COUNT(*) as count, SUM(price) as revenue, COALESCE(SUM(commission), 0) as commission')
            ->groupBy('date')
            ->orderBy('date')
            ->get();
    ...
    /**
     * Sync voucher usage status from RADIUS accounting data.
     */
    public function syncUsageWithRadius(): int
    {
        $radiusDb = $this->radius->getConnectionName();

        // 1. Find all available or sold vouchers
        $vouchers = Voucher::whereIn('status', ['Available', 'Sold'])->get();

        $count = 0;
        foreach ($vouchers as $voucher) {
            // 2. Check if this voucher has any record in radacct
            $hasActivity = DB::connection($radiusDb)->table('radacct')
                ->where('username', (string) $voucher->code)
                ->exists();

            if ($hasActivity) {
                // 3. Mark as used
                $success = $this->useVoucher($voucher->code, null);
                if ($success) {
                    $count++;
                }
            }
        ...
        if ($count > 0) {
            Log::info("Voucher usage sync completed: {$count} vouchers updated.");
        }
    ...
    public function resetVoucherCounter(Voucher $voucher): void
    {
        try {
            ...
            // \App\Services\Isp\Network\RadiusService::clearUsage($voucher->code);
        } catch (\Exception $e) {
