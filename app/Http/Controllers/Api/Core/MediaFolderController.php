...
class MediaFolderController extends BaseApiController
{
    ...
    protected function recursiveForceDelete(MediaFolder $folder): void
    {
        ...
        $mediaItems = \App\Models\Core\Media::where('folder_id', $folder->id)->withTrashed()->get();
        foreach ($mediaItems as $media) {
            /** @var \App\Models\Core\Media $media */
            $media->forceDelete();
        }

        // 2. Recurse into children folders
        $children = \App\Models\Core\MediaFolder::where('parent_id', $folder->id)->withTrashed()->get();
