...
{
    protected \App\Services\Core\CommentSecurityService $securityService;

    public function __construct(\App\Services\Core\CommentSecurityService $securityService)
    {
    ...
    public function store(Request $request, Content $content): \Illuminate\Http\JsonResponse
    {
        ...
        if ($user) {
            ...
            if (! \App\Models\Core\Setting::get('comments.security.allow_guests', true)) {
            ...
            if (\App\Models\Core\Setting::get('comments.security.guest_captcha', true)) {
                $captchaService = app(\App\Services\Core\CaptchaService::class);
