...
class MediaController extends BaseApiController
{
    ...
    public function upload(Request $request)
    {
        ...
        try {
            $maxSizeRaw = \App\Models\Core\Setting::get('max_upload_size', 10240);
    ...
    public function emptyTrash(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $folderQuery = \App\Models\Core\MediaFolder::onlyTrashed();
        if (! $user->can('manage media')) {
    ...
    public function bulkAction(Request $request)
    {
        ...
        $existingFolderIds = [];
        if (! empty($folderIds)) {
            $folderQuery = \App\Models\Core\MediaFolder::withTrashed()->whereIn('id', $folderIds);
            if (! $user->can('manage media')) {
                $folderQuery->where('author_id', $user->id);
            }
