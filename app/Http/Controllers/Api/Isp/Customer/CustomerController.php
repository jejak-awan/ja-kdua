...
class CustomerController extends BaseApiController
{
    ...
    protected \App\Services\Isp\Analytics\ChurnService $churnService;

    public function __construct(
        RouterService $routerService, 
        ISPValidationService $validator,
        \App\Services\Isp\Analytics\ChurnService $churnService
    ) {
    ...
    public function index(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $customers->getCollection()->transform(function ($user) {
            /** @var User $user */
            if ($user->customer) {
                /** @var \App\Models\Isp\Customer\Customer $customer */
                $customer = $user->customer;
                $customer->risk = $this->churnService->getRiskMetadata($customer);
            }
            return $user;
        });
    ...
    public function store(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            // Billing Data
            'billing_plan_id' => 'required|exists:isp_plans,id',
        ...
        if (isset($validated['identity_number'])) {
            $this->validator->validateHardwareIdentifer($validated['identity_number'], 'NIK');
        }
    ...
    public function update(Request $request, int $id): \Illuminate\Http\JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            // Billing Data
            'billing_plan_id' => 'sometimes|exists:isp_plans,id',
        ...
        try {
            ...
            if ($user->customer) {
                $oldStatus = $user->customer->status;
                $user->customer->update($customerData);
                $newStatus = $user->customer->status;

                // Lifecycle Synchronization: Handle manual status changes
                if ($oldStatus !== $newStatus) {
                    if ($newStatus === 'isolated') {
                        $this->routerService->suspendCustomer($user->customer);
                    } elseif ($newStatus === 'active') {
                        $this->routerService->reactivateCustomer($user->customer);
                    }
                }
            } else {
            ...
            // Provision on Mikrotik (Update properties like password/profile)
