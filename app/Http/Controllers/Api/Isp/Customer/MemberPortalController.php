...
class MemberPortalController extends BaseApiController
{
    ...
    public function dashboard(): \Illuminate\Http\JsonResponse
    {
        ...
        if ($customer && $customer->mikrotik_login) {
            ...
            if ($router) {
                /** @var \App\Services\Isp\Network\RouterService $routerService */
                $routerService = app(\App\Services\Isp\Network\RouterService::class);
    ...
    public function usage(): \Illuminate\Http\JsonResponse
    {
        ...
        if ($customer && $customer->mikrotik_login) {
            ...
            $usageResult = $this->mikrotik->getCustomerUsageHistory($cid);
    ...
    public function diagnostics(): \Illuminate\Http\JsonResponse
    {
        ...
        return $this->success($result, 'Diagnostics completed successfully');
    }

    /**
     * Kill the current session to "fix" connection issues.
     */
    public function resetConnection(): \Illuminate\Http\JsonResponse
    {
        /** @var \App\Models\Core\User|null $user */
        $user = Auth::user();
        if (! $user) {
        ...
        // 1. Try Radius CoA first (most common for modern setups)
        $coaSuccess = false;
        if ($customer->mikrotik_login) {
            $coaSuccess = $this->radius->sendDisconnectRequest($customer);
        }

        // 2. Try direct Router API disconnect
        $routerSuccess = false;
        if ($customer->mikrotik_login && $customer->router_id) {
            ...
            if ($router) {
                ...
                if ($session) {
                    $routerSuccess = $routerService->disconnectSession($router, $session['type'], $session['id']);
                }
            }
        }

        if ($coaSuccess || $routerSuccess) {
            return $this->success(null, 'Connection reset signal sent. Your device should reconnect in 10-20 seconds.');
        }

        return $this->success(null, 'No active session found to reset, or reset signal already processed.');
    }
}
