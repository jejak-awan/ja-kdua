...
namespace App\Http\Controllers\Api\Isp\Hardware;
...
class InventoryController extends BaseApiController
{
    ...
    /**
     * Generate QR code metadata for an item.
     */
    public function generateQr(Inventory $inventory): JsonResponse
    {
        // For now, we return the SKU and a deep link for the technician app/mobile web
        $deepLink = route('api.inventory.show', ['inventory' => $inventory->id]);
        
        return $this->success([
            'sku' => $inventory->sku,
            'name' => $inventory->name,
            'qr_data' => "ISP-INV:{$inventory->sku}",
            'link' => $deepLink
        ], 'QR metadata generated');
    }
