...
const regenerateAddresses = async () => {
  ...
  try {
    const { data } = await api.post(`/admin/ja/isp/ip-pools/${route.params.id}/regenerate`)
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2">
        <Button variant="outline" size="sm" @click="regenerateAddresses" :aria-label="t('isp.network.ip_pool.regenerate')">
          <RefreshCw class="mr-2 h-4 w-4" />
          {{ t('isp.network.ip_pool.regenerate') }}
        </Button>
        <Button size="sm" @click="router.push(`/admin/isp/network/ip-pools/${route.params.id}/edit`)">
    ...
    <div v-if="stats" class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <Card class="cursor-pointer hover:bg-accent/50" @click="filter = ''">
      ...
      <Card class="cursor-pointer hover:bg-accent/50" @click="filter = 'available'">
        <CardContent class="pt-4">
          <div class="flex items-center gap-3">
        ...
        <Table v-else>
          ...
          <TableBody>
            <TableRow v-for="addr in filteredAddresses" :key="addr.id">
              ...
              <TableCell>
                <Badge :variant="getStatusBadge(addr.status).variant">
