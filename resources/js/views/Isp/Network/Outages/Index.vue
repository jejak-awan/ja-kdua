<template>
    <div class="space-y-6 animate-in fade-in duration-500">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight">{{ t('isp.outages.title') }}</h1>
                <p class="text-muted-foreground">{{ t('isp.outages.subtitle') }}</p>
            </div>
    ...
    columnHelper.display({
        ...
        cell: info => {
            ...
            return h('div', { class: 'flex flex-col' }, [
                ...
                h('span', { class: 'text-[10px] text-muted-foreground uppercase mt-0.5' }, dayjs(outage.started_at).format('MMM DD, HH:mm'))
            ]);
        }
    }),
    columnHelper.display({
...
const fetchOutages = async () => {
    loading.value = true;
    try {
        const response = await api.get('/admin/ja/isp/outages');
...
const handleBulkAction = async (action: string) => {
    ...
    if (action === 'delete') {
        ...
        try {
            await Promise.all(ids.map(id => api.delete(`/admin/ja/isp/outages/${id}`)));
        ...
        try {
            await Promise.all(ids.map(id => api.patch(`/admin/ja/isp/outages/${id}`, { status: action })));
...
const saveOutage = async () => {
    ...
    try {
        ...
        const response = editId.value ? await api.patch(`/admin/ja/isp/outages/${editId.value}`, payload) : await api.post('/admin/ja/isp/outages', payload);
...
const deleteOutage = async (outage: Outage) => {
    if (!confirm(t('isp.outages.modals.delete_confirm'))) return;
    try {
        await api.delete(`/admin/ja/isp/outages/${outage.id}`);
        toast.success.default(t('isp.outages.messages.success_delete'));
        fetchOutages();
    } catch (_error) { toast.error.default(t('common.messages.error_delete')); }
};
