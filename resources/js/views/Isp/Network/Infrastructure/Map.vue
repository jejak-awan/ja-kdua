<template>
    <div>
        ...
        <div v-if="selectedNode" class="fixed bottom-10 right-10 z-[1000] w-72 transition-opacity">
            <Card class="p-4 shadow-2xl border border-border/40 bg-background/95 backdrop-blur rounded-xl">
                <div class="flex justify-between items-start mb-2">
                    <h3 class="font-bold text-lg">{{ selectedNode.name }}</h3>
                    <Button variant="ghost" size="icon" class="h-6 w-6 rounded-xl" @click="selectedNode = null">
                        <X class="w-4 h-4" />
                    </Button>
                </div>
                <div class="space-y-2">
                    <div class="flex justify-between text-sm">
                        ...
                        <Badge variant="outline" class="rounded-xl">{{ selectedNode.type }}</Badge>
                    </div>
                    <div class="flex justify-between text-sm">
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <Badge :variant="getStatusVariant(selectedNode.status)" class="rounded-xl">{{ selectedNode.status }}</Badge>
                    </div>
                    <div class="pt-2 border-t border-border/40 mt-2">
                        <router-link :to="{ name: 'isp.infra' }" class="w-full">
                            <Button variant="outline" size="sm" class="w-full h-8 text-[10px] rounded-xl">
...
const fetchNodes = async () => {
    ...
    try {
        const response = await api.get('/admin/janet/isp/infra', { params: { per_page: 100 } });
