<template>
    <div class="space-y-6 animate-in fade-in duration-500">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-3xl font-bold tracking-tight">{{ $t('isp.admin.odp.title', 'ODP Manager') }}</h2>
                <p class="text-muted-foreground">{{ $t('isp.admin.odp.subtitle', 'Manage Optical Distribution Points and port assignments') }}</p>
            </div>
            <Button @click="showCreateModal = true" class="rounded-xl">
        ...
        <Card class="border-border/40 shadow-sm">
            ...
            <CardContent class="p-0">
                    ...
                    <table class="w-full text-sm text-left">
                        ...
                        <tbody class="divide-y divide-border/40">
                            <tr v-for="odp in filteredOdps" :key="odp.id" class="hover:bg-muted/20 transition-colors group">
                                ...
                                <td class="px-6 py-4">
                                    <Badge variant="secondary" class="font-mono text-[10px] rounded-xl">
                                        {{ odp.olt?.name || $t('isp.infra.olt.discovery.unknown_parent') }}
                                    </Badge>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <Badge :variant="getStatusVariant(odp.status)" class="rounded-xl">{{ odp.status }}</Badge>
                                </td>
                                <td class="px-6 py-4 text-right opacity-0 group-hover:opacity-100 transition-opacity">
                                    <div class="flex items-center justify-end gap-2">
                                        <Button variant="ghost" size="icon" class="h-8 w-8 rounded-xl" @click="editOdp(odp)">
                                            <Pencil class="w-3.5 h-3.5" />
                                        </Button>
                                        <Button variant="ghost" size="icon" class="h-8 w-8 text-destructive rounded-xl" @click="deleteOdp(odp.id)">
                ...
                <div v-if="loading" class="p-12 flex flex-col items-center justify-center text-muted-foreground">
                    ...
                    <p class="text-sm">{{ $t('isp.infra.messages.loading') }}</p>
                </div>

                <div v-else-if="filteredOdps.length === 0" class="p-12 text-center text-muted-foreground">
                    <p class="text-sm">{{ $t('isp.infra.messages.no_data') }}</p>
                </div>
            </CardContent>
        </Card>
    </div>
...
const fetchData = async () => {
    loading.value = true;
    try {
        const response = await api.get('/admin/janet/isp/odps');
...
const deleteOdp = async (id: number) => {
    ...
    try {
        await api.delete(`/admin/janet/isp/odps/${id}`);
        await fetchData();
    } catch (error) {
