<template>
    <div class="space-y-6 animate-in fade-in duration-500">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card v-for="stat in quickStats" :key="stat.label" class="border-border/40 shadow-sm">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <!-- Main Traffic Chart -->
            <Card class="lg:col-span-2 border-border/40 shadow-sm">
                    ...
                    <div class="divide-y divide-border/40">
                        <div v-for="customer in topTalkers" :key="customer.id" class="flex items-center justify-between p-4 hover:bg-muted/30 transition-colors">
                            ...
                            <div class="text-right">
                                ...
                                <Badge variant="outline" class="text-[9px] h-4">{{ customer.status }}</Badge>
                            </div>
                        </div>
                    </div>
...
const fetchData = async () => {
    ...
    try {
        const [statsRes, usageRes, talkersRes] = await Promise.all([
            api.get('/admin/ja/isp/analytics'),
            api.get('/admin/ja/isp/analytics/usage'),
            api.get('/admin/ja/isp/analytics/top-talkers')
        ]);
