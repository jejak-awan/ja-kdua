<template>
    <div class="space-y-6 animate-in fade-in duration-500">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight">{{ t('isp.network.ipam.title') }}</h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div class="lg:col-span-1 space-y-4">
                <div class="flex items-center justify-between px-2">
                    <h3 class="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">{{ t('isp.network.subnets_manager.pool') }}</h3>
                ...
                <div class="space-y-2 max-h-[calc(100vh-280px)] overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    >
                        ...
                        <div class="flex items-center gap-2 mt-4">
                            <Badge variant="outline" class="text-[10px] py-0 h-4 border-muted-foreground/20 font-medium capitalize bg-background/50">
                                {{ subnet.node?.name || 'Unassigned' }}
                            </Badge>
                            <Badge variant="outline" class="text-[10px] py-0 h-4 border-muted-foreground/20 font-medium bg-background/50">
            ...
            <div class="lg:col-span-3">
                <div v-if="!selectedSubnet" class="h-96 flex flex-col items-center justify-center bg-card rounded-2xl border border-dashed border-border/60 text-muted-foreground/60 transition-colors hover:border-primary/20">
                ...
                <div v-else class="space-y-4 animate-in slide-in-from-right-2 duration-300">
                    <Card class="rounded-2xl border border-border shadow-sm overflow-hidden bg-card">
        ...
        <Dialog v-model:open="isIpModalOpen">
            <DialogContent class="sm:max-w-[420px] rounded-2xl border-none shadow-2xl">
                ...
                <form @submit.prevent="handleSaveIp" class="space-y-5 py-4">
                    <div class="space-y-2">
                        <Label class="text-xs font-bold uppercase tracking-wider text-muted-foreground">{{ t('isp.network.ip_manager.fields.status') }}</Label>
                    ...
                    <div class="space-y-2">
                        <Label class="text-xs font-bold uppercase tracking-wider text-muted-foreground">{{ t('isp.network.ip_manager.fields.notes') }}</Label>
                    ...
                    <div class="flex justify-between items-start pr-4">
                        <div class="space-y-1">
                            ...
                            <DialogDescription class="flex items-center gap-2">
                                <Badge variant="outline" class="rounded-md font-mono text-[10px]">{{ selectedSubnet?.prefix }}</Badge>
                                <span class="text-[10px] text-muted-foreground">•</span>
                                <span class="text-[10px] text-muted-foreground font-medium uppercase tracking-widest">VLAN {{ selectedSubnet?.vlan_id }}</span>
                            </DialogDescription>
                        </div>
                ...
                <div class="space-y-6 py-6">
                    <div v-if="selectedIp?.device" class="relative group p-6 bg-primary/5 rounded-3xl border border-primary/10 overflow-hidden">
                        ...
                        <div class="relative flex items-start gap-4">
                            <div class="p-3 bg-primary/20 rounded-2xl ring-4 ring-primary/5 shadow-inner">
                                <Globe class="w-6 h-6 text-primary" />
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-[10px] text-primary/70 uppercase tracking-[0.2em] font-black">Assigned Device</p>
                    ...
                    <div v-if="selectedIp?.notes" class="p-6 bg-muted/30 rounded-3xl border border-border/50">
                        <div class="flex items-center gap-2 mb-2 text-muted-foreground">
                            <div class="w-1.5 h-1.5 rounded-full bg-border"></div>
                            <span class="text-[10px] font-black uppercase tracking-[0.15em]">{{ t('isp.network.ip_manager.fields.notes') }}</span>
                        </div>
...
const columns = [
    ...
    columnHelper.display({
        ...
        cell: info => {
            ...
            return h('span', { class: 'text-[10px] text-muted-foreground font-bold tracking-[0.1em] opacity-30 uppercase' }, t('isp.network.ipam.available'));
        }
    }),
    columnHelper.display({
        id: 'actions',
...
const handleBulkAction = async (action: string) => {
    ...
    try {
        await Promise.all(ids.map(id => 
            api.patch(`/admin/ja/isp/network/subnets/${selectedSubnet.value!.id}/ips/${id}`, { status: action })
        ));
...
const fetchSubnets = async () => {
    loading.value = true;
    try {
        const response = await api.get('/admin/ja/isp/network/subnets');
...
const fetchNodes = async () => {
    try {
        const response = await api.get('/admin/ja/isp/infra');
        if (response.data.success) {
...
const fetchIps = async (subnetId: number, page = 1) => {
    ...
    try {
        ...
        const response = await api.get(`/admin/ja/isp/network/subnets/${subnetId}/ips`, { params });
...
const saveSubnet = async () => {
    ...
    try {
        const response = await api.post('/admin/ja/isp/network/subnets', subnetForm.value);
        if (response.data.success) {
...
const handleSaveIp = async () => {
    ...
    try {
        await api.patch(`/admin/ja/isp/network/subnets/${selectedSubnet.value.id}/ips/${selectedIp.value.id}`, ipForm.value);
        toast.success.action(t('isp.network.messages.success_ip'));
