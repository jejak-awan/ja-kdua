...
const fetchRouters = async () => {
  try {
    const { data } = await api.get('/admin/ja/isp/infra', { params: { type: 'Router' } })
...
const fetchPool = async () => {
  ...
  loading.value = true
  try {
    const { data } = await api.get(`/admin/ja/isp/ip-pools/${route.params.id}`)
...
const submit = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      await api.put(`/admin/ja/isp/ip-pools/${route.params.id}`, payload)
      toast.success.default(t('isp.network.ip_pool.messages.update_success'))
    } else {
      await api.post('/admin/ja/isp/ip-pools', payload)
...
<template>
  <div class="space-y-6">
    <div class="flex items-center gap-4">
      <Button variant="ghost" size="icon" @click="router.push('/admin/isp/network/ip-pools')" :aria-label="t('common.actions.back')">
        ...
        <form @submit.prevent="submit" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
              ...
              <Input id="name" v-model="form.name" placeholder="contoh: Pool Perumahan A" required />
            </div>

            <div class="space-y-2">
              <Label for="network">{{ t('isp.network.ip_pool.fields.network') }} *</Label>
              <Input id="network" v-model="form.network" :placeholder="t('isp.network.ip_pool.fields.network_placeholder')" required :disabled="isEdit" />
            ...
            <div class="space-y-2">
              ...
              <Input id="gateway" v-model="form.gateway" placeholder="contoh: 10.10.10.1" />
            </div>

            <div class="space-y-2">
              <Label for="vlan_id">{{ t('isp.network.ip_pool.fields.vlan_id') }}</Label>
              <Input id="vlan_id" v-model="form.vlan_id" type="number" min="1" max="4094" placeholder="1-4094" />
            </div>

            <div class="space-y-2">
              <Label for="dns_primary">{{ t('isp.network.ip_pool.fields.dns_primary') }}</Label>
              <Input id="dns_primary" v-model="form.dns_primary" placeholder="contoh: 8.8.8.8" />
            </div>

            <div class="space-y-2">
              <Label for="dns_secondary">{{ t('isp.network.ip_pool.fields.dns_secondary') }}</Label>
              <Input id="dns_secondary" v-model="form.dns_secondary" placeholder="contoh: 8.8.4.4" />
            </div>
            ...
            <div class="md:col-span-2 space-y-2">
              ...
              <Textarea id="description" v-model="form.description" :placeholder="t('isp.network.ip_pool.fields.description_placeholder')" rows="3" />
            </div>
          ...
          <div class="flex justify-end gap-2 pt-4 border-t">
            <Button type="button" variant="outline" @click="router.push('/admin/isp/network/ip-pools')">
