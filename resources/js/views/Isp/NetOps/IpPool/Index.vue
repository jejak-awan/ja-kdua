...
const fetchPools = async () => {
  loading.value = true
  try {
    const { data } = await api.get('/admin/ja/isp/ip-pools')
...
const deletePool = async (pool: IpPool) => {
  ...
  try {
    await api.delete(`/admin/ja/isp/ip-pools/${pool.id}`)
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2">
        <Button variant="outline" size="sm" @click="fetchPools" :aria-label="t('isp.network.ip_pool.refresh')">
          <RefreshCw class="mr-2 h-4 w-4" />
          {{ t('isp.network.ip_pool.refresh') }}
        </Button>
        <Button @click="router.push('/admin/isp/network/ip-pools/create')">
