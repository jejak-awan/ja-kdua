...
const fetchData = async () => {
    loading.value = true;
    try {
        const response = await api.get('/admin/janet/isp/print-templates');
...
const saveTemplate = async () => {
    ...
    try {
        if (editingTemplate.value) {
            await api.put(`/admin/janet/isp/print-templates/${editingTemplate.value.id}`, form.value);
            toast.success.action('Template updated');
        } else {
            await api.post('/admin/janet/isp/print-templates', form.value);
...
const previewTemplate = async (id: number) => {
    try {
        const response = await api.get(`/admin/janet/isp/print-templates/${id}/preview`);
...
const setDefault = async (tpl: Template) => {
    try {
        await api.put(`/admin/janet/isp/print-templates/${tpl.id}`, { is_default: true });
...
const deleteTemplate = async (id: number) => {
    ...
    if (!confirmed) return;
    try {
        await api.delete(`/admin/janet/isp/print-templates/${id}`);
