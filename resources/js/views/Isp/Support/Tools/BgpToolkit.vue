...
const lookupAsn = async () => {
    ...
    prefixes.value = [];
    try {
        const response = await api.get('/admin/ja/isp/tools/bgp/lookup', { params: { asn: asnInput.value } });
...
const fetchPrefixes = async () => {
    ...
    try {
        const response = await api.get('/admin/ja/isp/tools/bgp/prefixes', { params: { asn: asnInput.value } });
...
const downloadAddressList = () => {
    ...
    window.open(`/api/core/admin/ja/isp/tools/bgp/download-address-list?asn=${encodeURIComponent(asn)}`, '_blank');
};
</script>
