                ...
                <div class="space-y-6 py-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="p-3 bg-muted/40 rounded-xl">
                            <p class="text-[10px] text-muted-foreground uppercase font-bold opacity-60">{{ t('isp.support.fields.category') }}</p>
                            <p class="text-sm font-medium">{{ selectedTicket?.category }}</p>
                        </div>
                        <div class="p-3 bg-muted/40 rounded-xl">
                            <p class="text-[10px] text-muted-foreground uppercase font-bold opacity-60">{{ t('isp.support.fields.priority') }}</p>
                            <p class="text-sm font-medium">{{ selectedTicket?.priority }}</p>
                        </div>
                    </div>

                    <div class="space-y-2">
                        <Label class="text-xs font-bold text-muted-foreground uppercase tracking-wider">{{ t('isp.support.fields.message') }}</Label>
                        <div class="p-4 bg-muted/20 rounded-2xl border border-border/50 text-sm whitespace-pre-wrap leading-relaxed">
                            {{ selectedTicket?.message }}
                        </div>
                    </div>

                    <div class="flex items-center justify-between text-[10px] text-muted-foreground uppercase font-bold opacity-60 px-1">
...
const fetchTickets = async (page = 1) => {
    ...
    try {
        ...
        const response = await api.get('/admin/ja/isp/support/tickets', { params });
...
const updateTicketStatus = async (id: number, status: string) => {
    try {
        const response = await api.patch(`/admin/ja/isp/support/tickets/${id}/status`, { status });
...
const handleBulkAction = async (action: string) => {
    ...
    try {
        await Promise.all(ids.map(id => api.patch(`/admin/ja/isp/support/tickets/${id}/status`, { status: action })));
