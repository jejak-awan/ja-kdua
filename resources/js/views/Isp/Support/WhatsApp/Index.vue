...
const fetchData = async () => {
    ...
    try {
        const [tplRes, blastRes, schedRes] = await Promise.all([
            api.get('/admin/ja/isp/wa/templates'),
            api.get('/admin/ja/isp/wa/blasts'),
            api.get('/admin/ja/isp/wa/schedules')
        ]);
...
const saveTemplate = async () => {
    ...
    try {
        if (editingTemplate.value) {
            await api.put(`/admin/ja/isp/wa/templates/${editingTemplate.value.id}`, templateForm.value);
            toast.success.action('Template updated');
        } else {
            await api.post('/admin/ja/isp/wa/templates', templateForm.value);
...
const deleteTemplate = async (id: number) => {
    ...
    if (!confirmed) return;
    try {
        await api.delete(`/admin/ja/isp/wa/templates/${id}`);
...
const createBlast = async () => {
    ...
    try {
        ...
        await api.post('/admin/ja/isp/wa/blasts', payload);
...
const sendBlast = async (id: number) => {
    ...
    if (!confirmed) return;
    try {
        await api.post(`/admin/ja/isp/wa/blasts/${id}/send`);
...
const deleteBlast = async (id: number) => {
    ...
    if (!confirmed) return;
    try {
        await api.delete(`/admin/ja/isp/wa/blasts/${id}`);
...
const saveSchedule = async () => {
    ...
    try {
        ...
        if (editingSchedule.value) {
            await api.put(`/admin/ja/isp/wa/schedules/${editingSchedule.value.id}`, payload);
            toast.success.action('Schedule updated');
        } else {
            await api.post('/admin/ja/isp/wa/schedules', payload);
...
const toggleSchedule = async (sched: Schedule) => {
    try {
        await api.put(`/admin/ja/isp/wa/schedules/${sched.id}`, { is_active: !sched.is_active });
...
const deleteSchedule = async (id: number) => {
    ...
    if (!confirmed) return;
    try {
        await api.delete(`/admin/ja/isp/wa/schedules/${id}`);
