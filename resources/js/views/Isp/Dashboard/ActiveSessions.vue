        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div class="flex items-center gap-3 w-full md:w-auto">
                <Select v-model="selectedRouterId">
                    <SelectTrigger class="w-full md:w-[280px] bg-background/50 backdrop-blur-sm rounded-xl">
    ...
    columnHelper.accessor('name', {
        ...
        cell: ({ row }) => h('div', { class: 'flex items-center gap-3' }, [
            ...
            h('div', { class: 'flex flex-col' }, [
                ...
                h('span', { class: 'text-[10px] text-muted-foreground font-semibold opacity-60' }, row.original.service || 'Default')
            ])
        ])
    }),
...
const fetchRouters = async () => {
    try {
        const res = await api.get('/admin/janet/isp/routers');
...
const fetchSessions = async () => {
    ...
    loading.value = true;
    try {
        const res = await api.get(`/admin/janet/isp/monitor/sessions?router_id=${selectedRouterId.value}`);
...
const disconnect = async (session: Session) => {
    ...
    try {
        await api.post('/admin/janet/isp/monitor/disconnect', {
