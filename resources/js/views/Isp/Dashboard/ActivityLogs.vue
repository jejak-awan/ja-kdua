<template>
    <div class="space-y-6 animate-in fade-in duration-500">
        ...
        <!-- Filters -->
        <Card class="border-border/40 shadow-sm rounded-xl">
            <CardContent class="p-4">
                <div class="flex flex-wrap items-center gap-3">
                    <div class="relative flex-1 min-w-[200px] max-w-xs">
                        <Search class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                        <Input v-model="filters.search" placeholder="Search description..." class="pl-9 rounded-xl" @input="debouncedFetch" />
                    </div>
                    <Select v-model="filters.action" @update:model-value="() => fetchData()">
                        <SelectTrigger class="w-[140px] rounded-xl"><SelectValue placeholder="All Actions" /></SelectTrigger>
                    ...
                    <Input v-model="filters.from" type="date" class="w-[140px] rounded-xl" @change="fetchData" />
                    <Input v-model="filters.to" type="date" class="w-[140px] rounded-xl" @change="fetchData" />
                </div>
            </CardContent>
        </Card>
            ...
            <CardContent class="p-0">
                ...
                <div v-if="pagination.lastPage > 1" class="flex items-center justify-between p-4 border-t border-border/20">
                    ...
                    <div class="flex gap-1">
                        <Button variant="outline" size="sm" :disabled="pagination.currentPage <= 1" class="rounded-xl" @click="goToPage(pagination.currentPage - 1)">Prev</Button>
                        <Button variant="outline" size="sm" :disabled="pagination.currentPage >= pagination.lastPage" class="rounded-xl" @click="goToPage(pagination.currentPage + 1)">Next</Button>
                    </div>
                </div>
            </CardContent>
        </Card>
...
const fetchData = async (page = 1) => {
    ...
    try {
        ...
        const response = await api.get('/admin/janet/isp/activity-logs', { params });
