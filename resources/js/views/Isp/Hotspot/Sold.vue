...
const columns = [
    ...
    columnHelper.accessor('profile', {
        ...
        cell: ({ row }) => h(Badge, { variant: 'secondary', class: 'rounded-xl text-[10px]' }, () => row.original.profile)
    }),
...
const refundVoucher = async (id: number) => {
    ...
    try {
        await api.post(`/admin/janet/isp/vouchers/${id}/refund`);
...
const resetCounter = async (id: number) => {
    ...
    try {
        await api.post(`/admin/janet/isp/vouchers/${id}/reset-counter`);
        toast.success.action(t('common.messages.success'));
        await fetchData();
    } catch (error) {
        toast.error.action(error);
...
const bulkRefund = async () => {
    ...
    try {
        await api.post('/admin/janet/isp/vouchers/bulk-refund', { ids: selectedVoucherIds.value });
        toast.success.action(t('common.messages.success'));
        rowSelection.value = {};
        await fetchData();
    } catch (error) {
...
const bulkResetCounter = async () => {
    ...
    try {
        await api.post('/admin/janet/isp/vouchers/bulk-reset-counter', { ids: selectedVoucherIds.value });
        toast.success.action(t('common.messages.success'));
        rowSelection.value = {};
        await fetchData();
    } catch (error) {
...
<template>
    <div class="space-y-6 animate-in fade-in duration-500">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-3xl font-bold tracking-tight text-foreground">{{ $t('isp.admin.vouchers.sold_title', 'Sold Voucher Analysis') }}</h2>
                <p class="text-sm text-muted-foreground mt-1">{{ $t('isp.admin.vouchers.sold_subtitle', 'Track and manage reseller voucher sales and performance') }}</p>
            </div>
        </div>
            ...
            <CardHeader class="p-4 border-b border-border/40">
                <div class="flex flex-wrap items-center gap-4">
                    ...
                    <Select v-model="partnerFilter">
                        <SelectTrigger class="w-[180px] h-9 rounded-xl">
                    ...
                    <div class="ml-auto flex items-center gap-2">
                        ...
                        <Button variant="outline" size="sm" class="h-9 rounded-xl" @click="exportData">
