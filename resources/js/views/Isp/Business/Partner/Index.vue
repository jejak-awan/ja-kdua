...
const fetchData = async () => {
    loading.value = true;
    try {
        const response = await api.get('/api/core/isp/partners');
...
const savePartner = async () => {
    ...
    try {
        if (editingPartner.value) {
            await api.put(`/api/core/isp/partners/${editingPartner.value.id}`, form.value);
            toast.success.action(t('isp.admin.partner.updated', 'Partner updated'));
        } else {
            await api.post('/api/core/isp/partners', form.value);
...
const processSaldo = async () => {
    ...
    try {
        ...
        await api.post(`/api/core/isp/partners/${selectedPartner.value.id}/${endpoint}`, saldoForm.value);
...
const deletePartner = async (id: number) => {
    ...
    try {
        await api.delete(`/api/core/isp/partners/${id}`);
