    ...
    <div class="space-y-6 animate-in fade-in duration-500">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <Button variant="outline" @click="showSingleModal = true">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card v-for="(stat, idx) in summaryStats" :key="idx" class="border-border/40 shadow-sm overflow-hidden relative">
                <div :class="['absolute top-0 left-0 w-1 h-full', stat.accent]" />
                <CardContent class="p-5 flex items-center justify-between">
                    <div>
                        <p class="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">{{ stat.label }}</p>
                ...
                <div class="space-y-4 py-4">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <Input v-model="form.prefix" placeholder="e.g. VIP-" maxlength="10" />
                        </div>
                    </div>
        ...
        <Dialog v-model:open="showImportModal">
            <DialogContent class="sm:max-w-[425px]">
                ...
                <div class="p-6 border-2 border-dashed border-border rounded-lg text-center space-y-2 hover:border-primary/50 transition-colors cursor-pointer" @click="triggerFileInput">
                        ...
                        <div class="text-sm font-medium">{{ importFile ? importFile.name : $t('isp.admin.vouchers.select_file', 'Select or Drop File') }}</div>
                        <div class="text-xs text-muted-foreground">XLSX, XLS, CSV format supported</div>
            ...
            <DialogContent class="sm:max-w-[425px]">
                ...
                <div class="space-y-4 py-4">
                    ...
                    <Alert variant="destructive">
                        ...
                        <AlertDescription>This action cannot be undone. Vouchers will be permanently deleted.</AlertDescription>
                    </Alert>
                </div>
                <DialogFooter>
...
const fetchData = async () => {
    ...
    try {
        const [vItems, pItems, statsRes, mitraRes] = await Promise.all([
            ...
            api.get('/admin/ja/isp/vouchers/stock-summary'),
...
const bulkDelete = async () => {
    ...
    try {
        await api.post('/admin/ja/isp/vouchers/bulk-delete', { ids: selectedVoucherIds.value });
...
const bulkDeactivate = async () => {
    ...
    try {
        await api.post('/admin/ja/isp/vouchers/bulk-update-status', { 
...
const createSingleVoucher = async () => {
    ...
    try {
        ...
        await api.post('/admin/ja/isp/vouchers/create-single', payload);
...
const generateVouchers = async () => {
    ...
    try {
        ...
        const res = await api.post('/admin/ja/isp/vouchers/generate', payload);
...
const exportScript = () => {
    ...
    window.location.href = `/api/core/admin/ja/isp/vouchers/export/script?${params.toString()}`;
};
...
const importVouchers = async () => {
    ...
    try {
        await api.post('/admin/ja/isp/vouchers/import', formData, {
...
const runCleanup = async () => {
    submitting.value = true;
    try {
        const res = await api.post('/admin/ja/isp/vouchers/cleanup', cleanupForm.value);
