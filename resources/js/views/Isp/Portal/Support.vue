        ...
        <div class="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
            ...
            <div v-else class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead>
                        <tr class="text-xs text-muted-foreground bg-muted/30">
            ...
            <DialogContent class="sm:max-w-[600px] rounded-2xl">
                ...
                <div class="space-y-6 py-4">
                    ...
                    <div class="space-y-2 p-4 bg-muted/30 rounded-xl border border-border/50">
                        <span class="text-xs font-bold tracking-tight text-muted-foreground">{{ t('isp.support.fields.message') }}</span>
                    ...
                    <div v-if="selectedTicket?.admin_response" class="space-y-2 p-4 bg-primary/5 rounded-xl border border-primary/10">
                        <span class="text-xs font-bold tracking-tight text-primary">{{ t('isp.support.fields.response') }}</span>
...
const fetchTickets = async () => {
    loading.value = true;
    try {
        const response = await api.get('/admin/janet/isp/support/tickets');
...
const submitTicket = async () => {
    ...
    try {
        await api.post('/admin/janet/isp/support/tickets', form.value);
