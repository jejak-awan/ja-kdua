...
const fetchData = async (page = 1) => {
    loading.value = true;
    try {
        const response = await api.get('/admin/janet/isp/member/invoices', { params: { page, per_page: 10 } });
...
const handlePay = async (invoice: IspInvoice) => {
    try {
        const response = await api.post(`/admin/janet/isp/payments/invoice/${invoice.id}/create`);
