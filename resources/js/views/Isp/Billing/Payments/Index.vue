...
const loadPayments = async () => {
    loading.value = true;
    try {
        const response = await api.get('/admin/janet/isp/payments', { params: { ...filters, page: pagination.currentPage } });
...
const exportPayments = () => {
    window.open(`/api/v1/admin/janet/isp/payments/export?${new URLSearchParams(filters as Record<string, string>).toString()}`, '_blank');
};
