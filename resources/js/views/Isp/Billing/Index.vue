<template>
    <div class="space-y-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-foreground">{{ $t('isp.billing.title') }}</h1>
                <p class="text-sm text-muted-foreground">{{ $t('isp.billing.subtitle') }}</p>
            </div>
            <div class="flex gap-2">
                ...
                <Button @click="$router.push('/admin/isp/settings?tab=billing')" variant="outline" class="gap-2">
                    <Settings class="w-4 h-4" />
                    {{ $t('isp.billing.settings.title') }}
                </Button>
                <Button @click="generateInvoices" :disabled="generating" variant="secondary" class="gap-2">
                ...
                <Button @click="runSuspendCheck" :disabled="suspending" variant="destructive" class="gap-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card class="p-4 border-l-4 border-l-primary">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs text-muted-foreground uppercase tracking-wider font-bold">{{ $t('isp.billing.stats.revenue') }}</p>
                        <p class="text-xl font-bold">Rp {{ formatNumber(stats.total_revenue) }}</p>
                    </div>
                </div>
            </Card>
            <Card class="p-4 border-l-4 border-l-warning">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs text-muted-foreground uppercase tracking-wider font-bold">{{ $t('isp.billing.stats.pending') }}</p>
                        <p class="text-xl font-bold">Rp {{ formatNumber(stats.pending_amount) }}</p>
                    </div>
                </div>
            </Card>
            <Card class="p-4 border-l-4 border-l-success">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs text-muted-foreground uppercase tracking-wider font-bold">{{ $t('isp.billing.stats.paid') }}</p>
                        <p class="text-xl font-bold">{{ stats.paid_count }}</p>
                    </div>
                </div>
            </Card>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <Card v-for="plan in plans" :key="plan.id" class="p-6 relative overflow-hidden group">
                        <div class="flex justify-between items-start mb-4">
                            ...
                            <Badge :variant="plan.type === 'prepaid' ? 'success' : 'secondary'">
                                {{ plan.type }}
                            </Badge>
                        </div>
...
import type { IspInvoice, BillingPlan } from '@/types/isp';
...
const handleBulkAction = async (action: string) => {
    ...
    try {
        if (action === 'paid') {
            await Promise.all(ids.map(id => api.post(`/admin/ja/isp/billing/invoices/${id}/pay`)));
        } else {
...
const fetchData = async () => {
    loading.value = true;
    try {
        const statsRes = await api.get('/admin/ja/isp/billing/stats');
        stats.value = statsRes.data.data;

        const plansRes = await api.get('/admin/ja/isp/billing/plans');
...
const generateInvoices = async () => {
    ...
    try {
        const res = await api.post('/admin/ja/isp/billing/generate');
...
const runSuspendCheck = async () => {
    ...
    try {
        const res = await api.post('/admin/ja/isp/billing/suspend-check');
...
const handleSavePlan = async () => {
    ...
    try {
        ...
        if (editPlanId.value) {
            await api.patch(`/admin/ja/isp/billing/plans/${editPlanId.value}`, payload);
        } else {
            await api.post('/admin/ja/isp/billing/plans', payload);
        }
