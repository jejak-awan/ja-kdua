<template>
    <div class="space-y-6 animate-in fade-in duration-500">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="text-3xl font-bold tracking-tight text-foreground">{{ $t('isp.admin.coupons.title', 'Discount Coupons') }}</h2>
        ...
        <div class="grid gap-4 md:grid-cols-4">
            <Card v-for="stat in summaryStats" :key="stat.label" class="border-border/40 shadow-sm rounded-xl">
                <CardContent class="p-5">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs font-bold text-muted-foreground tracking-tight opacity-60">{{ stat.label }}</p>
                            <p class="text-2xl font-bold font-mono tracking-tighter">{{ stat.value }}</p>
                        </div>
            ...
            <CardHeader class="p-4 border-b border-border/40">
                <div class="flex flex-wrap items-center gap-4">
                    <div class="relative flex-1 max-w-xs">
                        ...
                        <Input v-model="search" type="text" class="pl-10 rounded-xl" :placeholder="$t('common.placeholders.search', 'Search code or name...')" />
                    </div>
...
const fetchData = async () => {
    loading.value = true;
    try {
        const response = await api.get('/admin/janet/isp/coupons');
...
const saveCoupon = async () => {
    ...
    try {
        if (editingCoupon.value) {
            await api.put(`/admin/janet/isp/coupons/${editingCoupon.value.id}`, form.value);
            toast.success.action(t('isp.admin.coupons.updated', 'Coupon updated'));
        } else {
            await api.post('/admin/janet/isp/coupons', form.value);
...
const toggleActive = async (coupon: Coupon) => {
    try {
        await api.put(`/admin/janet/isp/coupons/${coupon.id}`, { is_active: !coupon.is_active });
...
const deleteCoupon = async (id: number) => {
    ...
    if (!confirmed) return;
    try {
        await api.delete(`/admin/janet/isp/coupons/${id}`);
