...
>
            ...
            <div class="space-y-4">
                <Card class="p-4 bg-muted/30 border-none shadow-none">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-4">{{ t('isp.member.diagnostics.quick_actions') }}</h4>
                    <div class="space-y-2">
                ...
                <Alert class="rounded-2xl border-none bg-primary/5">
                    ...
                    <AlertTitle class="text-[10px] font-bold uppercase tracking-wider text-primary mb-1">{{ t('common.did_you_know') }}</AlertTitle>
...
const steps = ref([
    ...
    { name: 'DNS Resolution', status: 'pending', message: 'Verifying nameservers...', latency: '' },
...
const runDiagnostics = async () => {
    ...
    try {
        const response = await api.post('/admin/ja/isp/member/diagnostics');
        const results = response.data.data;

        // Artificial delay for better UX
        for (let i = 0; i < steps.value.length; i++) {
            await new Promise(resolve => setTimeout(resolve, 800));
            steps.value[i].status = results.steps[i].status;
            steps.value[i].message = results.steps[i].message;
            steps.value[i].latency = results.steps[i].latency || '';
        }

        overallStatus.value = results.overall_status;
...
const resetConnection = () => {
    toast.info('Connection Reset', 'Refreshing your internet session. Please wait 10-20 seconds.');
