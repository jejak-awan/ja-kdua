...
const profiles = ref<BillingPlan[]>([]);
const loading = ref(false);
const search = ref('');
const isModalOpen = ref(false);
const isDeleteAlertOpen = ref(false);
const isSaving = ref(false);
const selectedProfile = ref<BillingPlan | null>(null);
const profileToDelete = ref<BillingPlan | null>(null);

const columnHelper = createColumnHelper<BillingPlan>();
...
const columns = [
    ...
    columnHelper.accessor('type', {
        ...
        cell: info => h(Badge, { variant: info.getValue() === 'prepaid' ? 'secondary' : 'default' }, () => info.getValue()),
    }),
...
const fetchProfiles = async () => {
    loading.value = true;
    try {
        const response = await api.get('/admin/ja/isp/subscription-profiles', {
...
const openEditModal = (profile: BillingPlan) => {
...
const handleSave = async (formData: Record<string, unknown>) => {
    ...
    try {
        if (selectedProfile.value) {
            await api.put(`/admin/ja/isp/subscription-profiles/${selectedProfile.value.id}`, formData);
        } else {
            await api.post('/admin/ja/isp/subscription-profiles', formData);
        }
...
const promptDelete = (profile: BillingPlan) => {
...
const confirmDelete = async () => {
    ...
    try {
        await api.delete(`/admin/ja/isp/subscription-profiles/${profileToDelete.value.id}`);
