...
const loadContracts = async () => {
    loading.value = true;
    try {
        const response = await api.get('/admin/janet/isp/contracts', { params: { ...filters, page: pagination.currentPage } });
...
const loadFormData = async () => {
    try {
        const [customersRes, plansRes] = await Promise.all([
            api.get('/admin/janet/isp/customers'),
            api.get('/admin/janet/isp/subscription-profiles')
        ]);
...
const saveContract = async () => {
    saving.value = true;
    try {
        const url = editingContract.value ? `/admin/janet/isp/contracts/${editingContract.value.id}` : '/admin/janet/isp/contracts';
...
const downloadContract = (contract: Contract) => {
    window.open(`/api/v1/admin/janet/isp/contracts/${contract.id}/pdf`, '_blank');
};

const renewContract = async (contract: Contract) => {
    try {
        await api.post(`/admin/janet/isp/contracts/${contract.id}/renew`);
...
const terminateContract = async (contract: Contract) => {
    ...
    try {
        await api.post(`/admin/janet/isp/contracts/${contract.id}/terminate`);
