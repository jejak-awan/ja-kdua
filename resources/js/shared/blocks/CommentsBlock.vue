<template>
  <BaseBlock 
    ...
    :aria-label="(settings.aria_label as string) || 'Comments Section'"
  >
    <div class="comments-container w-full" :style="containerStyles">
      ...
      <div class="comments-list space-y-12 mb-20">
        <article v-for="comment in displayComments" :key="comment.id" class="comment-item group flex gap-8 p-10 bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border border-slate-50 dark:border-slate-800 transition-[width] duration-500 hover:-translate-y-2">
          <div v-if="(settings.showAvatar as boolean) !== false" class="comment-avatar-wrapper relative flex-shrink-0" :style="avatarStyles">
          ...
          <div class="comment-content flex-1 max-w-3xl">
            <header class="comment-header flex items-center justify-between mb-4">
              ...
              <button 
                  ...
                  @click="handleReply(comment.id as number)"
              >
                  Reply
      ...
      <div v-if="settings.showForm !== false" class="comment-form-section p-12 bg-white dark:bg-slate-900/50 rounded-[3rem] border border-dashed border-slate-200 dark:border-slate-800 overflow-hidden relative">
        ...
        <h4 class="text-2xl font-black tracking-tighter mb-10 text-slate-900 dark:text-white" :style="mainTitleStyles">
            {{ (settings.formTitle as string) || 'Add to the conversation' }}
        </h4>
        <form @submit.prevent class="relative z-10">
            <div class="form-grid grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div class="field-wrapper">
                  <Input 
                    type="text" 
                    :placeholder="(settings.namePlaceholder as string) || 'John Doe'" 
              ...
              <div class="field-wrapper">
                  <Input 
                    type="email" 
                    :placeholder="(settings.emailPlaceholder as string) || 'hello@modern.agency'" 
                    class="h-14 px-8 bg-slate-50 dark:bg-slate-950 border-none rounded-[1.5rem] font-medium focus-visible:ring-primary/20"
                    :style="fieldStyles" 
                  />
              </div>
            ...
            <textarea 
                :placeholder="(settings.commentPlaceholder as string) || 'Share your insights...'" 
                rows="5" 
...
// Mock dynamic data
const injectedComments = inject<Comment[]>('postComments', [
  ...
  { id: 2, author: 'Elena Vance', avatar: 'https://i.pravatar.cc/150?u=2', date: 'JAN 22, 2026', text: 'I love how the layout controls are baked right into the common settings. This makes brand consistency so much easier across large projects.' },
...
const containerStyles = computed((): CSSProperties => getLayoutStyles(settings.value, device.value) as CSSProperties)

const avatarStyles = computed((): CSSProperties => {
    const s = getVal<number>(settings.value, 'avatarSize', device.value) || 64
    return {
        width: `${s}px`,
        height: `${s}px`
    } as CSSProperties
...
const mainTitleStyles = computed(() => getTypographyStyles(settings.value, 'title_', device.value) as CSSProperties)
...
const metaStyles = computed(() => getTypographyStyles(settings.value, 'meta_', device.value) as CSSProperties)
const fieldStyles = computed(() => getTypographyStyles(settings.value, 'field_', device.value) as CSSProperties)
const buttonStyles = computed(() => getTypographyStyles(settings.value, 'button_', device.value) as CSSProperties)
</script>
